...
mapview(data, layer.name = 
          "Future dams under<br/>pessimistic scenario for 2050<br/>–––––––––––––––––––––––––––<br/>Change in scarcity risk (color)<br/>Hydropower capacity (size)",
        zcol = "RC1_P50rc", cex = data$Capacity, label = data$Project_na,  
        col.regions = my_col.regions,
        at = legendBreaks,
        alpha.regions = 0.8,
        lwd = 0.1,
        viewer.suppress = FALSE,
        popup = leafpop::popupTable(data, zcol = c("Project_na", "Capacity__", "Country", "Basin", "RC1", "RC1_P50", "RC1_P50rc"))
) %>%
  leafem::addLogo( 
...
mapview(data, layer.name = 
          "Future dams under<br/>pessimistic scenario for 2050<br/>–––––––––––––––––––––––––––<br/>Change in flood risk (color)<br/>Hydropower capacity (size)",
        zcol = "RC2_P50rc", cex = data$Capacity, label = data$Project_na,  
        col.regions = my_col.regions,
        at = legendBreaks,
        alpha.regions = 0.8,
        lwd = 0.1,
        viewer.suppress = FALSE,
        popup = leafpop::popupTable(data, zcol = c("Project_na", "Capacity__", "Country", "Basin", "RC2", "RC2_P50", "RC2_P50rc"))
) %>% 
  leafem::addLogo( 
  #"https://panda.maps.arcgis.com/sharing/rest/content/items/eaf9bbe5243d4dca8073e3517517bd71/data", src = "remote", # Black
  "https://panda.maps.arcgis.com/sharing/rest/content/items/203af4ae947245948ad0958bc2c7ea9c/data", src = "remote", # White
...
mapview(data, layer.name = 
          "Future dams risks to<br/>freshwater biodiversity<br/>–––––––––––––––––––––––––––<br/>Biodiversity risk in 2020 (color)<br/>Hydropower capacity (size)",
        zcol = "RC10", cex = data$Capacity, label = data$Project_na,  
        col.regions = colorRampPalette(c("#e9ffbe", "#ffd700", "#e60000")), 
        at = c(1, 1.4, 1.8, 2.2, 2.6, 3, 3.4, 3.8, 4.2, 4.6, 5),
        alpha.regions = 0.7,
        lwd = 0.1,
        viewer.suppress = FALSE,
        popup = leafpop::popupTable(data, zcol = c("Project_na", "Capacity__", "Country", "Basin", "RC10"))
) %>% 
  leafem::addLogo( 
  #"https://panda.maps.arcgis.com/sharing/rest/content/items/eaf9bbe5243d4dca8073e3517517bd71/data", src = "remote", # Black
  "https://panda.maps.arcgis.com/sharing/rest/content/items/203af4ae947245948ad0958bc2c7ea9c/data", src = "remote", # White
